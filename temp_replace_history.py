...
text = path.read_text()
start_marker = 'function renderExportHistory()'
end_marker = 'function describeHistoryEntry'
...
function renderHistoryDetails(entryId) {
 ...
 historyDetails.innerHTML = ;
  ...
  const rangeLabel = t ? t(exports_history_range_label) : Range;
  const exportedOnLabel = t ? t(exports_history_exported_on) : Exported on;
  const { formatLabel, langLabel } = describeHistoryEntry(entry);
  const startDate = entry.startDate || ---;
  const endDate = entry.endDate || ---;
